...
export default function DashboardPage() {
  ...
  async function handleDeleteFloor(floorId: string, floorName: string) {
    if (!window.confirm(`Delete "${floorName}"? All tables and walls on it will be permanently removed.`)) return;
          ...
          <div style={{ background: "#ffffff", borderRadius: "12px", padding: "2rem", width: "100%", maxWidth: "560px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", color: "#9a9088", fontSize: "1.25rem", lineHeight: 1, fontFamily: "inherit" }}
              >
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleEditRestaurant} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.625rem", marginTop: "0.25rem" }}>
                ...
                <button type="button" onClick={() => setIsEditingRestaurant(false)} className="btn btn-ghost btn-md">
                  Cancel
                </button>
              </div>
            </form>
            ...
            <div className="anim-1" style={{ opacity: 0, marginBottom: "2rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                  <button className="btn btn-ghost btn-sm" onClick={openEditRestaurant}>
                    Edit info
                  </button>
                  <Link
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: "1.5rem", alignItems: "start" }}>
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", minWidth: 0 }}>
                          <span style={{ fontSize: "0.6875rem", fontWeight: 600, color: s.color, background: s.bg, padding: "0.2rem 0.5rem", borderRadius: "999px", flexShrink: 0 }}>
                            {s.label}
                          </span>
                          <div style={{ minWidth: 0 }}>
                            <span style={{ fontSize: "0.9375rem", fontWeight: 500, color: "#18160f" }}>{floor.name}</span>
                            <span style={{ fontSize: "0.75rem", color: "#9a9088", marginLeft: "0.5rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.625rem", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", flex: 1 }}>
                        ...
                        <input
                          ...
                          max={2000}
                          value={newFloorWidth}
                          onChange={(e) => setNewFloorWidth(Math.max(200, +e.target.value))}
                          className="input"
                          style={{ flex: 1 }}
                        />
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", flex: 1 }}>
                        ...
                        <input
                          ...
                          onChange={(e) => setNewFloorHeight(Math.max(200, +e.target.value))}
                          className="input"
                          style={{ flex: 1 }}
                        />
                      </div>
              ...
              <div className="anim-4 card" style={{ opacity: 0, padding: "1.5rem" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
                  <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#18160f" }}>
                    Reservations
                  </h2>
                  {reservations.length > 12 && (
                    <span style={{ fontSize: "0.75rem", color: "#9a9088" }}>
                      Showing 12 of {reservations.length}
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                  {reservations.slice(0, 12).map((r, i) => (
                    <div
                      key={r.id}
                      style={{ padding: "0.875rem 0", borderBottom: i < Math.min(reservations.length, 12) - 1 ? "1px solid rgba(24,22,15,0.06)" : "none", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.75rem" }}
                    >
                      <div>
