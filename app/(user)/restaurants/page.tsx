      ...
      <div className="card-hover" style={{ overflow: "hidden" }}>
        ...
        <div style={{ padding: "1.25rem 1.25rem 1.5rem" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "0.75rem", marginBottom: "0.625rem" }}>
            <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#18160f", letterSpacing: "-0.01em", lineHeight: 1.3 }}>{r.name}</h2>
            {r.cuisine && (
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.5rem", flexWrap: "wrap" }}>
            ...
            {r.avgStars != null ? (
              <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                ...
                <span style={{ fontSize: "0.75rem", color: "#9a9088" }}>{Number(r.avgStars).toFixed(1)} ({r.reviewCount})</span>
              </span>
            ) : (
              <span style={{ fontSize: "0.75rem", color: "#c8c4be" }}>{t("restaurants.noReviews")}</span>
            )}
          </div>
          <div style={{ marginTop: "0.75rem", textAlign: "right" }}>
            <span style={{ fontSize: "0.8125rem", fontWeight: 600, color: "#c4410c" }}>{t("restaurants.reserve")}</span>
          </div>
        </div>
      </div>
    </Link>
    ...
    <div style={{ background: "#f5f3ef", minHeight: "100vh" }}>
      <nav className="nav">
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem", height: "64px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Link href="/" style={{ fontSize: "1.25rem", fontWeight: 700, color: "#18160f", textDecoration: "none", letterSpacing: "-0.02em" }}>mesa</Link>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            <LanguageSwitcher />
            {user ? (
              <>
                <Link href="/reservations" style={{ fontSize: "0.875rem", fontWeight: 500, color: "#5c5248", textDecoration: "none" }}>{t("nav.myReservations")}</Link>
                <button className="btn btn-ghost btn-sm" onClick={logout}>{t("nav.signOut")}</button>
              </>
            ) : (
              <>
                <Link href="/login" className="btn btn-ghost btn-sm" style={{ textDecoration: "none" }}>{t("nav.signIn")}</Link>
                <Link href="/register" className="btn btn-primary btn-sm" style={{ textDecoration: "none" }}>{t("nav.signUp")}</Link>
              </>
            )}
          </div>
        </div>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "3rem 1.5rem" }}>
        <div className="anim-1" style={{ opacity: 0, marginBottom: "2.5rem" }}>
          ...
          <div style={{ position: "relative", maxWidth: "480px" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9a9088" strokeWidth="2" style={{ position: "absolute", left: "0.875rem", top: "50%", transform: "translateY(-50%)" }}>
        ...
        {!loading && (
          <p style={{ fontSize: "0.875rem", color: "#9a9088", marginBottom: "1.5rem" }}>
            {filtered.length === 0
              ? t("restaurants.noFound")
              : t("restaurants.count", { count: filtered.length })}
            {search && ` ${t("restaurants.forQuery", { query: search })}`}
          </p>
        )}
        ...
        {!loading && filtered.length === 0 && restaurants.length === 0 && (
          <div style={{ textAlign: "center", padding: "5rem 0" }}>
            ...
            <p style={{ fontSize: "1.125rem", fontWeight: 600, color: "#18160f", marginBottom: "0.5rem" }}>{t("restaurants.noRestaurantsYet")}</p>
            <p style={{ fontSize: "0.9375rem", color: "#9a9088" }}>{t("restaurants.beFirst")}</p>
          </div>
        )}
      </div>
    </div>
