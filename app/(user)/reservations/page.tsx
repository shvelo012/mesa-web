...
export default function MyReservationsPage() {
  ...
  }, [user, _hasHydrated]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        {!loading && reservations.length === 0 && (
          <div className="card" style={{ padding: "4rem", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "1.125rem", fontWeight: 600, color: "#18160f", marginBottom: "0.5rem" }}>{t("reservations.noYet")}</p>
            <p style={{ fontSize: "0.9375rem", color: "#9a9088", marginBottom: "1.5rem" }}>{t("reservations.noYetSub")}</p>
            <Link href="/restaurants" className="btn btn-primary btn-md" style={{ textDecoration: "none" }}>{t("reservations.findTable")}</Link>
          </div>
        )}
        ...
        {upcoming.length > 0 && (
          <div className="anim-2" style={{ opacity: 0, marginBottom: "2rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
              {upcoming.map((r) => {
                const statusLabel = t(`reservations.status.${r.status}`, { defaultValue: r.status });
                return (
                  <div key={r.id} className="card" style={{ padding: "1.125rem 1.25rem", display: "flex", alignItems: "center", gap: "1.25rem" }}>
                    <div style={{ flexShrink: 0, width: "52px", height: "52px", background: "#fef2ec", borderRadius: "10px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontSize: "1.125rem", fontWeight: 700, color: "#c4410c", lineHeight: 1 }}>{r.date?.split("-")[2]}</span>
                      <span style={{ fontSize: "0.625rem", fontWeight: 600, color: "#c4410c", textTransform: "uppercase", marginTop: "1px" }}>
                        {r.date ? new Date(r.date + "T00:00").toLocaleString(undefined, { month: "short" }) : ""}
                      </span>
                    </div>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: "0.9375rem", fontWeight: 600, color: "#18160f" }}>{fmt(r.date)}</p>
                      <p style={{ fontSize: "0.8125rem", color: "#9a9088", marginTop: "0.15rem" }}>
                        {r.startTime} · {t("reservations.table")} {r.table?.label} · {t("reservations.guests", { count: r.partySize })}
                      </p>
                      {r.notes && <p style={{ fontSize: "0.8125rem", color: "#5c5248", marginTop: "0.15rem", fontStyle: "italic" }}>{r.notes}</p>}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.875rem", flexShrink: 0 }}>
                      <span className={`badge badge-${r.status.toLowerCase()}`}>{statusLabel}</span>
                      {r.status === "PENDING" && (
                        <button onClick={() => cancel(r.id)} className="btn btn-sm" style={{ background: "#fef2f2", color: "#dc2626", border: "none", fontFamily: "inherit" }}>
              ...
              {past.map((r) => {
                ...
                return (
                  <div key={r.id} className="card" style={{ padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1.25rem", opacity: 0.65 }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: "0.875rem", fontWeight: 500, color: "#18160f" }}>{fmt(r.date)} · {r.startTime}</p>
                      <p style={{ fontSize: "0.8125rem", color: "#9a9088", marginTop: "0.1rem" }}>
                        {t("reservations.table")} {r.table?.label} · {t("reservations.guests", { count: r.partySize })}
                      </p>
                    </div>
                    <span className={`badge badge-${r.status.toLowerCase()}`}>{statusLabel}</span>
                  </div>
