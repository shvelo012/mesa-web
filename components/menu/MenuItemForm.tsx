...
interface Props {
  onSave: (data: { name: string; price: number; description: string; dietaryTags: DietaryTag[] }) => Promise<void>;
  onCancel: () => void;
...
export default function MenuItemForm({ onSave, onCancel, initial }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await onSave({ name: name.trim(), price: parsedPrice, description: description.trim(), dietaryTags });
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      {error && (
        <div style={{ padding: "0.5rem 0.75rem", background: "#fef2f2", border: "1px solid rgba(220,38,38,0.2)", borderRadius: "6px", color: "#dc2626", fontSize: "0.8125rem" }}>{error}</div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: "0.625rem" }}>
        <div>
          <label className="label">Dish name</label>
          <input required value={name} onChange={(e) => setName(e.target.value)} className="input" placeholder="e.g. Grilled Salmon" />
        </div>
        <div>
      ...
      <div>
        ...
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} className="input" placeholder="Short description…" style={{ resize: "vertical" }} />
      </div>
      <div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  cursor: "pointer",
                  border: active ? "1px solid #c4410c" : "1px solid rgba(24,22,15,0.16)",
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.25rem" }}>
        <button type="submit" disabled={saving} className="btn btn-primary btn-sm">{saving ? "Saving…" : initial?.id ? "Update dish" : "Add dish"}</button>
